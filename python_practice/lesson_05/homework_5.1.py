...
year_min, engine_min, max_price = search_criteria

result = []

for brand, car_info in car_data.items():
    color, year, engine, car_type, price = car_info

    if year >= year_min and engine >= engine_min and price <= max_price:
        result.append((brand, car_info))
